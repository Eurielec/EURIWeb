...
export default function CompletarPerfilClient({
  ...
  const isFormCompleted = state?.success;
    ...
    <main className="min-h-screen flex items-start justify-center bg-brand overflow-y-auto relative pt-24 pb-12 px-4 font-sans">
      ...
      <div className="relative z-10 w-full max-w-2xl p-8 sm:p-10 card-light m-4 my-12 shadow-2xl backdrop-blur-2xl">
        <div className="text-center mb-8 relative">
          ...
          <h1 className="text-3xl font-black text-black uppercase tracking-tighter italic mb-2">
            {isFormCompleted ? 'Cuota de' : t.auth.completeProfile.titlePart1} <span className="text-white">{isFormCompleted ? 'Socio' : t.auth.completeProfile.titlePart2}</span>
          </h1>
          <p className="text-black/60 text-xs font-bold tracking-widest uppercase mt-2">
            {isFormCompleted ? 'Último paso antes de entrar' : t.auth.completeProfile.subtitle}
          </p>
        </div>
        ...
        {isFormCompleted ? (
          <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500">
            ...
            <Link 
              href="/perfil"
              className="w-full max-w-md py-4 bg-black/10 hover:bg-black/20 text-black border border-black/20 font-black text-[10px] uppercase tracking-[0.3em] rounded-2xl transition-all active:scale-[0.98] text-center"
            >
              Continuar al Perfil
            </Link>
          </div>
